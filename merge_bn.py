...
def make_parser():
    ...
    parser.add_argument('--model', type=str, required=False, help='.prototxt file for inference')
    parser.add_argument('--weights', type=str, required=False, help='.caffemodel file for inference')
    return parser
...
def pre_process(expected_proto, new_proto):
    layer_white = ["bn6f", "bn6f_scale", "bn6w", "bn6w_scale", "fc1", "fc1_scale"]
    ...
    for idx in range(num_layers):
        ...
        if l.type == "BatchNorm" or l.type == "Scale":
            ...
            if l.name not in layer_white:
                continue
            else:
                print(l.name, l.name not in layer_white)
                layer = net_specs2.layer.add()
                layer.MergeFrom(l)
        elif l.type == "Convolution" or l.type == "Deconvolution" or l.type == "BatchNorm" or l.type == "Scale":
...
if __name__ == '__main__':
    ...
    args.model = "mobilefacenet-v0.prototxt"
    args.weights = "mobilefacenet-v0.caffemodel"
